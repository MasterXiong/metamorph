...
def evaluate_checkpoint(folder, test_set, interval=600, additional_suffix=None):
    ...
    while (1):
        ...
        if not os.path.exists(model_path):
            break
        evaluate_model(model_path, agent_path, policy_folder, suffix=suffix, compute_gae=True)
        iteration += interval
...
if __name__ == '__main__':
    ...
    # python tools/eval_learning_curve.py --folder output/ft_400M_generate_validation_curate_uniform_KL_5_wo_PE+dropout/1409 --test_set unimals_100/test --interval 1800 --suffix wo_reward_update
